...
# Source code preprocessor for the TRD build process.
# Credit: KACTL
# License: CC0
...
def addref(caption, outstream):
    ...
    print(r"\trdref{%s}" % caption, file=outstream)
...
def processwithcomments(caption, instream, outstream, listingslang):
    ...
    out = []
    if warning:
        out.append(r"\trdwarning{%s: %s}" % (caption, warning))
    if error:
        out.append(r"\trderror{%s: %s}" % (caption, error))
    else:
...
def processraw(caption, instream, outstream, listingslang = 'raw'):
    try:
        ...
        print(r"\trderror{Could not read source.}", file=outstream)
